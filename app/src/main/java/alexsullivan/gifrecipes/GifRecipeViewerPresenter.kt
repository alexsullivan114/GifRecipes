...
class GifRecipeViewerPresenterImpl(private val gifRecipe: GifRecipeUI,
  ...
  private fun loadInitialPlayingState() {
    ...
    val buildPlayingState = fun(): GifRecipeViewerViewState {
      return if (gifRecipe.imageType == ImageType.GIF) {
        PlayingGif(cacheServer.get(gifRecipe.url), gifRecipe)
      } else {
        PlayingVideo(cacheServer.get(gifRecipe.url), gifRecipe)
      }
    }

    val buildLoadingState = fun(progress: Int): GifRecipeViewerViewState {
      return if (gifRecipe.imageType == ImageType.GIF) {
        LoadingGif(progress, gifRecipe, url = cacheServer.get(gifRecipe.url))
      } else {
        LoadingVideo(progress, gifRecipe, url = cacheServer.get(gifRecipe.url), hasTransitioned = false)
      }
    }
    ...
    if (cacheServer.isCached(gifRecipe.url)) {
      ...
      if (gifRecipe.imageType.isGif()) {
        cacheServer.prefetch(gifRecipe.url).subscribe()
      }
