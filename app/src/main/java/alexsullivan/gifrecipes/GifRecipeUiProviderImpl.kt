...
class GifRecipeUiProviderImpl(private val repository: GifRecipeRepository,
  ...
  override fun fetchRecipes(count: Int): Observable<Pair<Observable<GifRecipeRepository.Response>, List<GifRecipeUI>>> {
  ...
  override fun fetchRecipes(continuation: Observable<GifRecipeRepository.Response>): Observable<Pair<Observable<GifRecipeRepository.Response>, List<GifRecipeUI>>> {
    return continuation
        .flatMap { response ->
          response.recipes.toGifRecipeUi().map { response.continuation to it }
        }
